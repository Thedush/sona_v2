...
def color_transform_hsv(image):
    ...
    global robot_state
    if robot_state == 3:
        lower_hls = np.array([117,0, 160])
        upper_hls = np.array([160, 255,255]) 
    else:
        lower_hls = np.array([55,0, 63])
        upper_hls = np.array([95, 255,225])
    if robot_state == 2:
        lower_hls = np.array([140,0, 0])   # Define lower and upper HSL values for masking
        upper_hls = np.array([179, 255,230])
...
def slide_window(binary_warped, histogram):
    ...
    for window in range(nwindows):
        ...
        cropped_img = binary_warped[win_y_low:win_y_high, win_xleft_low:win_xleft_high]
        non_zero_line = np.count_nonzero(cropped_img)
        if non_zero_line > line_threshold:
    ...
    line_percentage = line_count/nwindows
    ...
    try:
       ...
       # parameters = aruco.DetectorParameters_create()
        ...
        if robot_state:
            ...
            empty_message = Twist()
            #empty_message.linear.x = 0.0
            #empty_message.angular.z = 0.0
...
def main():
    ...
    global publisher,cap,stop_publisher
