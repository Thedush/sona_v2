...
def image_callback(msg):
    ...
    try:
        ...
        if line_percentage == 0 :
            message.data[0] = int(PreviousError)
          
        else :
            message.data[0] = error
        
       
        message.data[1] = int(line_percentage * 100)
        message.data[2] = 0
...
def main():
    ...
    global publisher,cap,stop_publisher,pid_publisher,error_publisher
    if PID_TUNING :
    ...
    error_publisher = node.create_publisher(Int16MultiArray, 'color_line', rclpy.qos.qos_profile_system_default)
